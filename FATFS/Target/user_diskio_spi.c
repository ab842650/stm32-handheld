...
#define FCLK_FAST() { MODIFY_REG(SD_SPI_HANDLE.Instance->CR1, SPI_BAUDRATEPRESCALER_256, SPI_BAUDRATEPRESCALER_4); }	/* Set SCLK = fast, approx 10.5 MBits/s */
...
{
	static BYTE txBuf[512];  /* dummy TX (SD card ignores MOSI during data response) */
	HAL_SPI_TransmitReceive(&SD_SPI_HANDLE, txBuf, buff, btr, HAL_MAX_DELAY);
}
